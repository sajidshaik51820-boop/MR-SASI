...
export async function preprocessImage(file) {
  ...
  if (!res.ok) {
    const err = await res
      .json()
      .catch(() => ({ detail: "Preprocessing failed" }));
...
export async function analyzeRemoteSensing(
  file,
  indexType = "VARI"
) {
  const formData = new FormData();
  ...
  if (!res.ok) {
    const err = await res
      .json()
      .catch(() => ({
        detail: "Remote sensing analysis failed",
      }));

    throw new Error(
      err.detail || "Remote sensing analysis failed"
    );
  }
...
export async function getCrops() {
  ...
  if (!res.ok) {
    throw new Error("Failed to load crops list");
  }
...
export async function getCropDetail(cropName) {
  const res = await fetch(
    `${API_BASE}/crops/${encodeURIComponent(cropName)}`
  );

  if (!res.ok) {
    throw new Error(
      `Failed to load crop details for ${cropName}`
    );
  }
...
export async function getGrowthFramework(cropName) {
  const res = await fetch(
    `${API_BASE}/growth-stages/${encodeURIComponent(cropName)}`
  );

  if (!res.ok) {
    throw new Error(
      `Failed to load growth framework for ${cropName}`
    );
  }
...
export async function getPestManagement() {
  const res = await fetch(
    `${API_BASE}/pest-management`
  );

  if (!res.ok) {
    throw new Error(
      "Failed to load pest management data"
    );
  }
...
export async function getDiseases() {
  ...
  if (!res.ok) {
    throw new Error(
      "Failed to load diseases directory"
    );
  }
...
export async function evaluateModel(
  datasetPath = "datasets/plantvillage"
) {
  const res = await fetch(
    `${API_BASE}/evaluate?dataset_path=${encodeURIComponent(
      datasetPath
    )}`
  );
